...
def split_audio(audio_data, sample_width=2, silence_threshold=0.001, sample_rate=16000, chunk_duration=1): 
...
def make_tarfile(source_dir, tarfile_dir):
    with tarfile.open(tarfile_dir, "w:gz") as tar: 
...
async def run(context, input, aws_key, aws_secret, aws_bucket):
    key = aws_key
    secret = aws_secret
    bucket = aws_bucket
    ...
    downloads = os.path.join(path, object)
    ...
    up_object = "temp/checkpoint.tar.gz" 
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    filename = "checkpoint"
    upload_filename = f"{filename}_{timestamp}.tar.gz"
    upload_object(key, secret, bucket, up_object, upload_filename)

    # list S3 bucket object 
    lst = checkpoint_search(key, secret, bucket)

    return streams.Stream.read_from(f"{lst}\n") 


# si start sequence - --args [\"aws_key, aws_secret, bucket\"]
# si inst input - /deep-learning/audio-training-seq/MK_2.1/multi-label-audio-02.wav -e -t application/octet-stream